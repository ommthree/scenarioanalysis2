...
# Migration: Update staged_file CHECK constraint to include conversion file type

DB_PATH="${1:-/Users/Owen/ScenarioAnalysis2/data/database/finmodel.db}"

echo "Updating staged_file table to support conversion file type..."
...
CREATE TABLE staged_file_new (
    ...
    file_type TEXT NOT NULL CHECK (file_type IN ('scenario', 'balance_sheet', 'pnl', 'carbon', 'cashflow', 'location', 'damage_curve', 'hazard_map', 'correlation', 'conversion')),
    file_path TEXT,
    row_count INTEGER,
...
echo "Migration complete! conversion file type added to staged_file table."
