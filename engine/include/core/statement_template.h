...
struct LineItem {
    ...
    bool is_computed;                           ///< True if pure within-period calculation (cannot have no_driver=true)
    bool no_driver;                             ///< True if uses BASE/[t-1] without driver multiplier (cannot have is_computed=true)
