/**
 * @file test_level7.cpp
 * @brief Level 7: Tax Calculations
 *
 * Level 7 extends Level 6 by adding tax calculations.
 *
 * Key additions over Level 6:
 * - PRETAX_INCOME (income before taxes)
 * - TAX_EXPENSE (tax on pretax income)
 * - NET_INCOME becomes after-tax
 * - TAX_PAYABLE on balance sheet (liability)
 * - CASH_TAX_PAYMENTS (working capital item)
 * - DELTA_TAX_PAYABLE in Operating CF
 *
 * Expected behavior:
 * - Tax expense reduces pretax income to get net income
 * - Not all tax is paid immediately → TAX_PAYABLE liability
 * - Tax payable changes affect operating cash flow
 * - Balance sheet includes tax liability
 */
...
// ============================================================================
// Level 7 Test - Tax Calculations
// ============================================================================

TEST_CASE("Level 7: Tax Calculations", "[level7]") {
    auto db = DatabaseFactory::create_sqlite("../data/database/finmodel.db");

    std::cout << "\n=== LEVEL 7: TAX CALCULATIONS ===" << std::endl;
    std::cout << "Testing: Pretax income, tax expense, and tax payable" << std::endl;
    std::cout << std::endl;

    // Test configuration
    EntityID entity_id = "TEST_ENTITY_L7";
    ScenarioID scenario_id = 1007;
    ...
    // Test data with tax calculations
    struct PeriodData {
        int period_id;
        double revenue;
        double cogs;
        ...
        double stock_comp;
        double interest;
        double tax_expense;  // NEW
        double debt_proceeds;
        double debt_repayment;
    };

    std::vector<PeriodData> test_periods = {
        // P1: Pretax = 80k, Tax = -20k (25% rate), NI = 60k
        {1, 100000.0, 0.0, -10000.0, -5000.0, -3000.0, 0.0, -2000.0, -20000.0, 50000.0, 0.0},
        // P2: Pretax = 88k, Tax = -22k (25% rate), NI = 66k
        {2, 110000.0, 0.0, -11000.0, -5000.0, -3000.0, 0.0, -3000.0, -22000.0, 0.0, 0.0},
        // P3: Pretax = 97k, Tax = -24k (~25% rate), NI = 73k
        {3, 120000.0, 0.0, -12000.0, -5000.0, -3000.0, 0.0, -3000.0, -24000.0, 0.0, -20000.0}
    };
    ...
    for (PeriodID period_id : period_ids) {
        ...
        // Display key metrics
        std::cout << "  PRETAX_INCOME = " << std::fixed << std::setprecision(2) << result.get_value("PRETAX_INCOME") << std::endl;
        std::cout << "  TAX_EXPENSE = " << result.get_value("TAX_EXPENSE") << std::endl;
        std::cout << "  NET_INCOME = " << result.get_value("NET_INCOME") << std::endl;
        std::cout << "  TAX_PAYABLE = " << result.get_value("TAX_PAYABLE") << std::endl;
    ...
    CSVExporter::export_unified_summary("test_output/level7_summary.csv", results, initial_bs);
    std::cout << std::endl;

    // Verify key calculations for Period 1
    double pretax1 = results[0].get_value("PRETAX_INCOME");
    double tax1 = results[0].get_value("TAX_EXPENSE");
    double ni1 = results[0].get_value("NET_INCOME");
    double tax_payable1 = results[0].get_value("TAX_PAYABLE");
    double cf_op1 = results[0].get_value("CASH_FLOW_OPERATING");
    double cash1 = results[0].get_value("CASH");

    // Pretax Income = Revenue - OPEX - Depreciation - Amortization - Interest
    // = 100,000 - 10,000 - 5,000 - 3,000 - 2,000 = 80,000
    REQUIRE(pretax1 == Approx(80000.0).margin(0.01));

    // Tax expense should be -20,000 (25% of 80k)
    REQUIRE(tax1 == Approx(-20000.0).margin(0.01));

    // Net Income = Pretax Income + Tax Expense (after-tax)
    // = 80,000 + (-20,000) = 60,000
    REQUIRE(ni1 == Approx(60000.0).margin(0.01));

    // Tax Payable = 0 - (-20,000) - (-18,000) = 20,000 - 18,000 = 2,000
    // (90% of tax is paid in cash: 20k * 0.90 = 18k)
    REQUIRE(tax_payable1 == Approx(2000.0).margin(0.01));

    // Operating CF now includes DELTA_TAX_PAYABLE
    // CF_OP = NI - Depreciation - Amortization - ΔAR + ΔTAX_PAYABLE
    // = 60,000 - (-5,000) - (-3,000) - 5,000 + 2,000 = 65,000
    // Note: This reflects the tax accrual/cash timing difference
    ...
    std::cout << "✓ Level 7 test complete!" << std::endl;
    std::cout << "  Real UnifiedEngine: ✓" << std::endl;
    std::cout << "  Tax calculations: ✓" << std::endl;
    std::cout << "  Pretax income: ✓" << std::endl;
    std::cout << "  Tax payable: ✓" << std::endl;
    std::cout << "  Tax timing in cash flow: ✓" << std::endl;
    std::cout << "  Open test_output/level7_summary.csv in Excel to verify" << std::endl;
}
